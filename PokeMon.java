...
Every time you subtract M from N that means you’ve reached a target and poked
 it successfully. COUNT how many targets you’ve poked – you’ll need that count.
...
public class PokeMon {
	public static void main(String[] args) {
		...
		int pokeMonDistansTargets = Integer.parseInt(scanner.nextLine());
		int thirdLineY = Integer.parseInt(scanner.nextLine());	
		int count = 0;
		int currentPower = pokeMonPower;		
		while (currentPower>=pokeMonDistansTargets) {
			currentPower-=pokeMonDistansTargets;
			count++;							
	  if (currentPower==pokeMonPower*0.5 && thirdLineY!=0) {
		 currentPower/=thirdLineY;					
			} 	
		}
